 ...
 */
const getChartConfig = (isMobile: boolean, chartType: ChartType = 'default'): ChartConfig => {
  // Base configuration
 ...
 * @param period - SMA period (10, 20, 50, etc.)
 * @param priceKey - The key to use for price (defaults to checking multiple possibilities)
 * @param outputKey - The property name to use for the output (defaults to sma{period})
 * @returns Array of data with SMA values added
 */
const calculateSMA = (
  ...
  let validSMACount = 0;
  result.forEach(item => {
    if (item[actualOutputKey] !== null && item[actualOutputKey] !== undefined && !isNaN(Number(item[actualOutputKey]))) {
      validSMACount++;
    }
  });
...
const processChartData = (
  ...
  let processedData: ProcessedStockDataPoint[] = chartData.map((item: unknown) => ({...item as ProcessedStockDataPoint}));
  ...
  processedData = processedData.map(item => {
    ...
    if (!('sma10' in result)) {
      ...
      else if ('ema10' in item) result.sma10 = item.ema10 as number | null; // Sometimes EMA is used instead
      else if ('EMA10' in item) result.sma10 = item.EMA10 as number | null;
      else if ('10sma' in item) result.sma10 = item['10sma'] as number | null; // New format
      else if ('10SMA' in item) result.sma10 = item['10SMA'] as number | null; // New format
    }
    ...
    if (!('sma20' in result)) {
      ...
      else if ('20sma' in item) result.sma20 = item['20sma'] as number | null; // New format
      else if ('20SMA' in item) result.sma20 = item['20SMA'] as number | null; // New format
    }
    ...
    if (!('sma50' in result)) {
      ...
      else if ('50sma' in item) result.sma50 = item['50sma'] as number | null; // New format
      else if ('50SMA' in item) result.sma50 = item['50SMA'] as number | null; // New format
    }
  ...
  const hasSMA10 = processedData.some(d => d.sma10 !== null && d.sma10 !== undefined && !isNaN(Number(d.sma10)));
  const hasSMA20 = processedData.some(d => d.sma20 !== null && d.sma20 !== undefined && !isNaN(Number(d.sma20)));
  const hasSMA50 = processedData.some(d => d.sma50 !== null && d.sma50 !== undefined && !isNaN(Number(d.sma50)));
...
const StockChart = React.memo<StockChartProps>(({ 
  ...
  const stockData = useMemo<ProcessedStockDataPoint[]>(() => processChartData(chartData, chartType), [chartData, chartType]);
  ...
  const hasSMA10 = stockData.some(item => item.sma10 !== null && item.sma10 !== undefined && !isNaN(Number(item.sma10)));
  const hasSMA20 = stockData.some(item => item.sma20 !== null && item.sma20 !== undefined && !isNaN(Number(item.sma20)));
  const hasSMA50 = stockData.some(item => item.sma50 !== null && item.sma50 !== undefined && !isNaN(Number(item.sma50)));
  ...
  const scales = useMemo<ChartScales | null>(() => {
    ...
    const mainValues = stockData.flatMap(d => {
      ...
      return values.filter((v): v is number => v !== null && v !== undefined && !isNaN(v) && v > 0.001); // Filter out zero or extremely small values
    });
  ...
  }, [dimensions, stockData, afterStockData, hasSMA10, hasSMA20, hasSMA50, visibleAfterData, zoomPercentage, isMobile, chartType, tightPadding, onChartClick]);
    ...
    return line<ProcessedStockDataPoint>()
      ...
      .y(d => {
        ...
        if (d.sma10 === null || d.sma10 === undefined || isNaN(Number(d.sma10))) {
          return NaN; // This point will be skipped in the path
        }
        const yPos = scales.priceScale(d.sma10 as number);
        return yPos;
      })
...
export { getChartConfig, calculateSMA, processChartData };
